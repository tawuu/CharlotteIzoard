class TicTacToeGame {
    ...
    async updateEmbed(message, channel, t, winner) {
        ...
        if ([0, 1].includes(winner)) {
            channel.createMessage({
                content: this.participantes[winner].mention,
                embed: {
                    ...
                    description: `:o: **${this.participantes[0].tag}**\n:x: **${this.participantes[1].tag}**\n\n` + t("commands:tictactoe.hasWinner", {
                        winner: this.participantes[winner].mention
                    }),
                    image: {
    ...
    async createCollector(cc, channel, t) {
        ...
        const collector = new this.client.utils.MessageCollector(channel, filter, {
            max: Infinity,
            time: 60000
        });
        collector.on("collect", async (msg) => {
            ...
            if (msg.author.id === this.participantes[this.vez].id) {
                ...
                if (this.isEmpty(this.game, cordinate)) {
                    ...
                    if (channel.guild.members.get(this.client.user.id).permission.has("manageMessages")) msg.delete()
                    cc.delete()
